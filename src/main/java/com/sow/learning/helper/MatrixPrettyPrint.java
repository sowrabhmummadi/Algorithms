...
public final class MatrixPrettyPrint{
    ...
    public MatrixPrettyPrint(PrintStream out) {
        this(out, DEFAULT_AS_NULL);
    }

    public MatrixPrettyPrint(PrintStream out, String asNull) {
        if ( out == null ) {
    ...
    public void print(int[][] table) {
        ...
        String[][] stringTable= new String[length][length];
        for (int i = 0; i < length; i++) {
            for (int j = 0; j < width; j++) {
                stringTable[i][j]=String.valueOf(table[i][j]);
            }
        }
        print(stringTable);
    }
